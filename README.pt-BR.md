# Manoela Calabresi · Portfólio técnico em português-BR

Após atuar em escritórios de arquitetura na Alemanha, fundei a Ombu Lab, iniciativa de inteligência territorial com atuação no Brasil. Este portfólio reúne projetos e casos voltados a mobilidade, análise espacial, infraestrutura de dados territoriais, indicadores censitários e decisão locacional.

## Casos principais

### 1. Utrecht · Hubs de mobilidade e lacunas territoriais

Estudo espacial sobre mobilidade compartilhada, acesso ao transporte público e cobertura territorial na Província de Utrecht.

- resultado principal: mapa de cobertura e lacunas;
- foco: mobilidade, acessibilidade e priorização territorial.

**Link final do caso:** [README.pt-BR do repositório Utrecht](https://github.com/Manoela-Calabresi-Portfolio/Utrecht-Mobility-Hub-Analysis/blob/main/README.pt-BR.md)

---

### 2. ETL de geodados · infraestrutura para bases territoriais

Pipeline reprodutível para baixar, limpar, padronizar e organizar bases geoespaciais abertas para uso em mapas, indicadores e análises territoriais.

- resultado principal: base tratada e reutilizável;
- foco: dados secundários, georreferenciamento e infraestrutura analítica.

**Link final do caso:** [README.pt-BR do repositório ETL](https://github.com/Manoela-Calabresi-Portfolio/ETL-Geodata-Pipeline/blob/main/README.pt-BR.md)

---

### 3. Atlas urbano de Curitiba · submercados e modelagem territorial

Caso de modelagem territorial em escala de cidade para definir submercados urbanos a partir de zoneamento, oferta, centralidades e eixos estruturais.

- resultado principal: definição de submercados territoriais;
- foco: leitura urbana, localização e estrutura analítica auditável.

**Link final do caso:** [Atlas urbano de Curitiba](casos/atlas-curitiba.pt-BR.md)

---

### 4. IBGE/SIDRA · indicadores censitários e leitura territorial

Estrutura para descobrir, coletar e organizar tabelas do IBGE/SIDRA e transformá-las em indicadores utilizáveis para análise espacial.

- resultado principal: base para indicadores de renda, domicílio e escolaridade;
- foco: integração entre censo, território e infraestrutura analítica.

**Link final do caso:** [IBGE/SIDRA e indicadores territoriais](casos/ibge-sidra.pt-BR.md)

---

## O que este portfólio demonstra

- análise de dados secundários;
- georreferenciamento e análise espacial;
- uso de bases públicas e censitárias;
- mobilidade, acessibilidade e leitura territorial;
- infraestrutura de dados urbanos;
- síntese técnica em linguagem acessível.

## Contato

- [LinkedIn](https://www.linkedin.com/in/manoela-calabresi/)
- [manoela.calabresi@gmail.com](mailto:manoela.calabresi@gmail.com)
- GitHub: [Manoela-Calabresi-Portfolio](https://github.com/Manoela-Calabresi-Portfolio)
